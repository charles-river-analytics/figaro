 ...
 */
class If[T](name: Name[T], val test: Element[Boolean], val thn: Element[T], val els: Element[T], collection: ElementCollection)
  extends CachingChain(name, test, (b: Boolean) => if (b) thn; else els, collection) {
