...
class RaisingVE(universe: Universe, targets: Element[_]*) extends StructuredAlgorithm(universe, targets:_*) {
 
  val semiring = SumProductSemiring()
  
  def run() {    
    val strategy = DecompositionStrategy.recursiveRaisingStrategy(problem, new ConstantStrategy(variableElimination), RaisingStrategy.raiseIfGlobal, defaultRangeSizer, Lower, false)
