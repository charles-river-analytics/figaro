...
class ParticleGenerator(de: DensityEstimator, val numArgSamples: Int, val numTotalSamples: Int) {
      ...
      case o: OneShifter => {
        ...
        toResample.map(b => {
          ...
          val nextValue = if (o.density(newValue._1) > 0.0) {
            accept(oldValue, newValue._1, newValue._2, beliefs.asInstanceOf[List[(Double, Int)]])
          } else oldValue
      ...
      case a: Atomic[_] => { // The double is unchecked, bad stuff if the atomic is not double
        beliefs.map(b => {
          ...
          val nextValue = if (a.asInstanceOf[Atomic[Double]].density(newValue) > 0.0) {
            accept(oldValue, newValue, 1.0, beliefs.asInstanceOf[List[(Double, Double)]])
          } else oldValue
          (sampleDensity, nextValue)
        })
      }
  ...
  private def accept[T](oldValue: T, newValue: T, proposalProb: Double, beliefs: List[(Double, T)]): T = {
