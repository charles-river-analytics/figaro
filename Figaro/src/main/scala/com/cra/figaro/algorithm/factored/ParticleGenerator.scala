...
/**
 * Class to handle sampling from continuous elements in PBP
 * @param numArgSamples Maximum number of samples to take from atomic elements
 * @param numTotalSamples Maximum number of samples on the output of chains
...
class ParticleGenerator(de: DensityEstimator, val numArgSamples: Int, val numTotalSamples: Int) {
  ...
  def apply[T](elem: Element[T]): List[(Double, T)] = {
    sampleMap.get(elem) match {
      ...
      case None => apply(elem, numArgSamples)
    }
  }
